...
describe("Scroll behavior", () => {
  ...
  const setupScrollTest = () =>
    Effect.gen(function* () {
      ...
      yield* Given.KHORA_IS_FOCUSED_AT(firstKhoraId, CURSOR_POSITION);
      ...
      return { scrollContainer, frameId, childNodeIds, firstKhoraId };
    });
    ...
    await Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        waitFor(() => {
          ...
          if (
            block.querySelectorAll("[data-element-type='khora']").length < 3
          ) {
      ...
      const fullBlockEl = document.querySelector<HTMLElement>(
        `[data-element-id="${parentBlockId}"][data-element-type="khora"]`,
      );
      const contentRowEl = document.querySelector<HTMLElement>(
        `[data-element-id="${parentBlockId}"][data-element-type="khora"] [data-block-content]`,
      );
