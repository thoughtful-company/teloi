...
describe("Block Cmd+Shift+Backspace (Force Delete)", () => {
  ...
  describe("in text editing mode", () => {
    ...
    it("moves focus to previous sibling after deletion", async () => {
      await Effect.gen(function* () {
        ...
        yield* Then.SELECTION_IS_ON_KHORA(
          Id.makeFrameKhoraId(frameId, firstNodeId),
        );
      }).pipe(runtime.runPromise);
    });
    ...
    it("moves focus to parent if no previous sibling", async () => {
      await Effect.gen(function* () {
        ...
        yield* Given.KHORA_IS_FOCUSED_AT(onlyChildBlockId, 0);
        yield* When.USER_PRESSES("{Meta>}{Shift>}{Backspace}{/Shift}{/Meta}");
  ...
  describe("in khora selection mode", () => {
...
describe("Regular Delete Automerge cleanup (Bug fix)", () => {
  ...
  it("Delete in khora selection mode cleans up Automerge text", async () => {
    await Effect.gen(function* () {
      const { frameId, childNodeIds } = yield* Given.A_FRAME_WITH_CHILDREN(
        "Root",
      ...
      // Enter khora selection mode on First
      yield* When.USER_ENTERS_KHORA_SELECTION(firstKhoraId);
      yield* Then.BLOCKS_ARE_SELECTED(frameId, [firstNodeId]);
  ...
  it("Backspace in khora selection mode cleans up Automerge text", async () => {
    await Effect.gen(function* () {
      const { frameId, childNodeIds } = yield* Given.A_FRAME_WITH_CHILDREN(
        "Root",
      ...
      // Enter khora selection mode on Second
      yield* When.USER_ENTERS_KHORA_SELECTION(secondBlockId);
