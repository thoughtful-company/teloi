...
describe("Shift+Up from in-block text selection", () => {
  ...
  it("enters khora selection mode when focus offset is 0", async () => {
  ...
  it("does NOT enter khora selection mode when focus offset is not 0", async () => {
    await Effect.gen(function* () {
      const { frameId, childNodeIds } =
        yield* Given.A_FRAME_WITH_CHILDREN("Root", [{ text: "Hello world" }]);
  ...
  it("enters khora selection from collapsed cursor at offset 0", async () => {
    await Effect.gen(function* () {
      const { frameId, childNodeIds } =
        yield* Given.A_FRAME_WITH_CHILDREN("Root", [{ text: "Hello world" }]);
