...
/**
 * Tests for document-order khora selection navigation.
 *
 * Current behavior: ArrowUp/ArrowDown in khora selection mode is confined to siblings.
 ...
 */
describe("Khora selection document-order navigation - ArrowUp", () => {
  ...
  it("ArrowUp from first child selects parent", async () => {
    await Effect.gen(function* () {
      ...
      // Enter khora selection mode on child A (first child)
  ...
  it("ArrowUp from block goes to prev sibling's deepest visible child", async () => {
    await Effect.gen(function* () {
      ...
      yield* Then.KHORA_IS_EXPANDED(Id.makeFrameKhoraId(frameId, nodeA));
      yield* Then.KHORA_IS_EXPANDED(Id.makeFrameKhoraId(frameId, nodeA1));

      // Enter khora selection mode on B
  ...
  it("ArrowUp at first block at root scrolls to top and keeps selection", async () => {
    await Effect.gen(function* () {
      ...
      // Enter khora selection mode on first block
...
describe("Khora selection document-order navigation - ArrowDown", () => {
  let runtime: BrowserRuntime;
  let render: Awaited<ReturnType<typeof setupClientTest>>["render"];
  let cleanup: () => Promise<void>;
  ...
  it("ArrowDown from expanded parent selects first child", async () => {
    await Effect.gen(function* () {
      ...
      // Enter khora selection mode on parent
  ...
  it("ArrowDown from last child goes to parent's next sibling", async () => {
    await Effect.gen(function* () {
      ...
      // Enter khora selection mode on A2 (last child of A)
      yield* When.USER_ENTERS_KHORA_SELECTION(blockA2);
  ...
  it("ArrowDown at last block at root keeps selection", async () => {
    await Effect.gen(function* () {
      ...
      // Enter khora selection mode on last block
      yield* When.USER_ENTERS_KHORA_SELECTION(lastKhoraId);
...
describe("Khora selection document-order navigation - Edge cases", () => {
  let runtime: BrowserRuntime;
  let render: Awaited<ReturnType<typeof setupClientTest>>["render"];
  let cleanup: () => Promise<void>;
  ...
  it("ArrowDown from deeply nested last child climbs up to find next", async () => {
    await Effect.gen(function* () {
      ...
      // Enter khora selection mode on A1a (deeply nested)
      yield* When.USER_ENTERS_KHORA_SELECTION(blockA1a);
      yield* Then.BLOCKS_ARE_SELECTED(frameId, [nodeA1a]);
