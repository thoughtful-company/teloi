...
describe("Block Mod+, key (ZoomOut)", () => {
  ...
  it("selects collapsed ancestor when zooming out from invisible child", async () => {
    await Effect.gen(function* () {
      ...
      const firstChildBlockId = Id.makeFrameKhoraId(frameId, firstChildNodeId);
      ...
      const grandchildBlockId = Id.makeFrameKhoraId(frameId, grandchildNodeId);

      render(() => <FrameView frameId={frameId} />);

      // Collapse "First child" block (hides grandchild in Root view)
      const Khora = yield* KhoraT;
      yield* Khora.setExpanded(firstChildBlockId, false);
      yield* Then.KHORA_IS_COLLAPSED(firstChildBlockId);
      ...
      yield* Given.KHORA_IS_FOCUSED_AT(grandchildBlockId, 0);
  ...
  it("scrolls cursor into view after each Mod+, zoom out", async () => {
    await Effect.gen(function* () {
      ...
      const isBlockVisibleInContainer = (khoraId: Id.Khora): boolean => {
      ...
      // Press Mod+, #1: Khora 22 view → Block 21 view (cursor on Block 22)
      yield* When.USER_PRESSES("{Meta>},{/Meta}");
      ...
      const block22BlockId = Id.makeFrameKhoraId(frameId, block22Id);
      yield* Effect.promise(() =>
        waitFor(
          () => {
            expect(
      ...
      // Press Mod+, #2: Khora 21 view → Block 20 view (cursor on Block 21)
      yield* When.USER_PRESSES("{Meta>},{/Meta}");
      ...
      const block21BlockId = Id.makeFrameKhoraId(frameId, block21Id);
      yield* Effect.promise(() =>
        waitFor(
          () => {
            expect(
      ...
      // Press Mod+, #3: Khora 20 view → Root view (cursor on Block 20)
      yield* When.USER_PRESSES("{Meta>},{/Meta}");
      ...
      const block20BlockId = Id.makeFrameKhoraId(frameId, block20Id);
      yield* Effect.promise(() =>
        waitFor(
          () => {
            expect(
