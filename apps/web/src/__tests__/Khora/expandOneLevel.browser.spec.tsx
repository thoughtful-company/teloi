...
/**
 * Tests for KhoraT.expandOneLevel function.
 *
 ...
 */
describe("KhoraT.expandOneLevel", () => {
  ...
  it("expands self when collapsed", async () => {
    await Effect.gen(function* () {
      ...
      // Verify collapsed
      const isExpandedBefore = yield* Khora.isExpanded(blockA);
      expect(isExpandedBefore).toBe(false);

      // When: expandOneLevel is called
      const result = yield* Khora.expandOneLevel(frameId, nodeA);

      // Then: Node is expanded and returns true
      expect(result).toBe(true);
      const isExpandedAfter = yield* Khora.isExpanded(blockA);
  ...
  it("expands first collapsed child when self is expanded", async () => {
    await Effect.gen(function* () {
      ...
      expect(yield* Khora.isExpanded(blockA)).toBe(true);
      expect(yield* Khora.isExpanded(blockA1)).toBe(false);
      ...
      expect(result).toBe(true);
      expect(yield* Khora.isExpanded(blockA1)).toBe(true);
    }).pipe(runtime.runPromise);
  });
  ...
  it("expands second collapsed child when first is already expanded", async () => {
    await Effect.gen(function* () {
      ...
      const blockA2 = Id.makeFrameKhoraId(frameId, nodeA2);
      ...
      const Khora = yield* KhoraT;
      yield* Khora.setExpanded(blockA2, false);

      // Verify initial state
      expect(yield* Khora.isExpanded(blockA2)).toBe(false);
      ...
      expect(result).toBe(true);
      expect(yield* Khora.isExpanded(blockA2)).toBe(true);
    }).pipe(runtime.runPromise);
  });
  ...
  it("goes to grandchildren when all immediate children are expanded", async () => {
    await Effect.gen(function* () {
      ...
      const blockA1a = Id.makeFrameKhoraId(frameId, nodeA1a);
      ...
      const Khora = yield* KhoraT;
      yield* Khora.setExpanded(blockA1a, false);

      // Verify initial state
      expect(yield* Khora.isExpanded(blockA1a)).toBe(false);
      ...
      expect(result).toBe(true);
      expect(yield* Khora.isExpanded(blockA1a)).toBe(true);
    }).pipe(runtime.runPromise);
  });
  ...
  it("returns false when node has no children and is already expanded", async () => {
    await Effect.gen(function* () {
      ...
      const blockLeaf = Id.makeFrameKhoraId(frameId, nodeLeaf);

      // Node is expanded by default (though it has no children)
      const Khora = yield* KhoraT;
      expect(yield* Khora.isExpanded(blockLeaf)).toBe(true);

      // When: expandOneLevel is called
      const result = yield* Khora.expandOneLevel(frameId, nodeLeaf);
  ...
  it("works with deeply nested hierarchies (3+ levels)", async () => {
    await Effect.gen(function* () {
      ...
      const blockA1a1 = Id.makeFrameKhoraId(frameId, nodeA1a1);
      ...
      yield* Khora.setExpanded(blockA1, false);
      yield* Khora.setExpanded(blockA1a, false);
      yield* Khora.setExpanded(blockA1a1, false);

      // First call: expands A1
      const result1 = yield* Khora.expandOneLevel(frameId, nodeA);
      expect(result1).toBe(true);
      expect(yield* Khora.isExpanded(blockA1)).toBe(true);
      expect(yield* Khora.isExpanded(blockA1a)).toBe(false);
      expect(yield* Khora.isExpanded(blockA1a1)).toBe(false);

      // Second call: expands A1a
      const result2 = yield* Khora.expandOneLevel(frameId, nodeA);
      expect(result2).toBe(true);
      expect(yield* Khora.isExpanded(blockA1a)).toBe(true);
      expect(yield* Khora.isExpanded(blockA1a1)).toBe(false);

      // Third call: expands A1a1
      const result3 = yield* Khora.expandOneLevel(frameId, nodeA);
      expect(result3).toBe(true);
      expect(yield* Khora.isExpanded(blockA1a1)).toBe(true);

      // Fourth call: nothing left to expand
      const result4 = yield* Khora.expandOneLevel(frameId, nodeA);
