...
describe("Shift+Down from in-block text selection", () => {
  ...
  it("enters khora selection mode when focus offset is at document length", async () => {
  ...
  it("does NOT enter khora selection mode when focus offset is not at document length", async () => {
    await Effect.gen(function* () {
      const { frameId, childNodeIds } =
        yield* Given.A_FRAME_WITH_CHILDREN("Root", [{ text: "Hello world" }]);
  ...
  it("enters khora selection from collapsed cursor at document length", async () => {
    await Effect.gen(function* () {
      const { frameId, childNodeIds } =
        yield* Given.A_FRAME_WITH_CHILDREN("Root", [{ text: "Hello world" }]);
