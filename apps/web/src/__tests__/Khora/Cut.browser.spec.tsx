...
describe("Cut selected blocks (Mod+X)", () => {
  ...
  it("Mod+X cuts single selected block (copies and deletes)", async () => {
    await Effect.gen(function* () {
      ...
      yield* When.USER_ENTERS_KHORA_SELECTION(khoraId);
      ...
      // And: Khora is deleted from DOM and model
  ...
  it("Mod+X cuts nested child block (copies and deletes)", async () => {
    await Effect.gen(function* () {
      ...
      const nestedBlockId = Id.makeFrameKhoraId(frameId, nestedChildA);
      yield* When.USER_ENTERS_KHORA_SELECTION(nestedBlockId);
  ...
  it("Mod+X cuts multiple nested children", async () => {
    await Effect.gen(function* () {
      ...
      const nestedBlockA = Id.makeFrameKhoraId(frameId, nestedChildA);
      yield* When.USER_ENTERS_KHORA_SELECTION(nestedBlockA);
      yield* When.USER_PRESSES("{Shift>}{ArrowDown}{/Shift}");
