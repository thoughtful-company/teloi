 ...
 *
 * Works in both editor mode (CodeMirror) and khora selection mode.
...
describe("Block Mod+Enter todo toggle", () => {
  ...
  describe("Khora selection mode", () => {
    it("converts normal block to unchecked checkbox in khora selection mode", async () => {
      await Effect.gen(function* () {
        const Type = yield* TypeT;
        const Tuple = yield* TupleT;
        ...
        // Enter khora selection mode (click then Escape)
        yield* When.USER_ENTERS_KHORA_SELECTION(childBlockId);
    ...
    it("toggles multiple selected blocks independently", async () => {
      await Effect.gen(function* () {
        ...
        const normalBlockId = Id.makeFrameKhoraId(frameId, normalId);
        ...
        yield* When.USER_ENTERS_KHORA_SELECTION(normalBlockId);
